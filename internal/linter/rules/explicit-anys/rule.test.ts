import { ruleTester } from '../rule-tester.ts'
import { rule } from './config.ts'

ruleTester.describe(rule, {
	valid: [
		`const number: number = 1;`,
		`function greet(): string {}`,
		`function greet(): Array<string> {}`,
		`function greet(): string[] {}`,
		`function greet(): Array<Array<string>> {}`,
		`function greet(): Array<string[]> {}`,
		`function greet(param: Array<string>): Array<string> {}`,
		`
class Greeter {
  message: string;
}
    `,
		`
class Greeter {
  message: Array<string>;
}
    `,
		`
class Greeter {
  message: string[];
}
    `,
		`
class Greeter {
  message: Array<Array<string>>;
}
    `,
		`
class Greeter {
  message: Array<string[]>;
}
    `,
		`
interface Greeter {
  message: string;
}
    `,
		`
interface Greeter {
  message: Array<string>;
}
    `,
		`
interface Greeter {
  message: string[];
}
    `,
		`
interface Greeter {
  message: Array<Array<string>>;
}
    `,
		`
interface Greeter {
  message: Array<string[]>;
}
    `,
		`
type obj = {
  message: string;
};
    `,
		`
type obj = {
  message: Array<string>;
};
    `,
		`
type obj = {
  message: string[];
};
    `,
		`
type obj = {
  message: Array<Array<string>>;
};
    `,
		`
type obj = {
  message: Array<string[]>;
};
    `,
		`
type obj = {
  message: string | number;
};
    `,
		`
type obj = {
  message: string | Array<string>;
};
    `,
		`
type obj = {
  message: string | string[];
};
    `,
		`
type obj = {
  message: string | Array<Array<string>>;
};
    `,
		`
type obj = {
  message: string & number;
};
    `,
		`
type obj = {
  message: string & Array<string>;
};
    `,
		`
type obj = {
  message: string & string[];
};
    `,
		`
type obj = {
  message: string & Array<Array<string>>;
};
    `,
		{
			code: `
        function foo(a: number, ...rest: any[]): void {
          return;
        }
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function foo1(...args: any[]) {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const bar1 = function (...args: any[]) {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const baz1 = (...args: any[]) => {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function foo2(...args: readonly any[]) {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const bar2 = function (...args: readonly any[]) {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const baz2 = (...args: readonly any[]) => {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function foo3(...args: Array<any>) {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const bar3 = function (...args: Array<any>) {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const baz3 = (...args: Array<any>) => {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function foo4(...args: ReadonlyArray<any>) {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const bar4 = function (...args: ReadonlyArray<any>) {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `const baz4 = (...args: ReadonlyArray<any>) => {};`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Qux1 {
  (...args: any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Qux2 {
  (...args: readonly any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Qux3 {
  (...args: Array<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Qux4 {
  (...args: ReadonlyArray<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quux1(fn: (...args: any[]) => void): void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quux2(fn: (...args: readonly any[]) => void): void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quux3(fn: (...args: Array<any>) => void): void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quux4(fn: (...args: ReadonlyArray<any>) => void): void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quuz1(): (...args: any[]) => void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quuz2(): (...args: readonly any[]) => void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quuz3(): (...args: Array<any>) => void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `function quuz4(): (...args: ReadonlyArray<any>) => void {}`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Fred1 = (...args: any[]) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Fred2 = (...args: readonly any[]) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Fred3 = (...args: Array<any>) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Fred4 = (...args: ReadonlyArray<any>) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Corge1 = new (...args: any[]) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Corge2 = new (...args: readonly any[]) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Corge3 = new (...args: Array<any>) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `type Corge4 = new (...args: ReadonlyArray<any>) => void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Grault1 {
  new (...args: any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Grault2 {
  new (...args: readonly any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Grault3 {
  new (...args: Array<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Grault4 {
  new (...args: ReadonlyArray<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Garply1 {
  f(...args: any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Garply2 {
  f(...args: readonly any[]): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Garply3 {
  f(...args: Array<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `
interface Garply4 {
  f(...args: ReadonlyArray<any>): void;
}
      `,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `declare function waldo1(...args: any[]): void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `declare function waldo2(...args: readonly any[]): void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `declare function waldo3(...args: Array<any>): void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
		{
			code: `declare function waldo4(...args: ReadonlyArray<any>): void;`,
			options: {
				ignoreRestArgs: true,
			},
		},
	],
	invalid: [
		{
			code: `const number: any = 1;`,
			snapshot: `const number: any = 1;
              ~~~
              Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `const number: unknown = 1;`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `const number: never = 1;`,
				},
			],
		},
		{
			code: `function generic(): any {}`,
			snapshot: `function generic(): any {}
                    ~~~
                    Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(): unknown {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(): never {}`,
				},
			],
		},
		{
			code: `function generic(): Array<any> {}`,
			snapshot: `function generic(): Array<any> {}
                          ~~~
                          Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(): Array<unknown> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(): Array<never> {}`,
				},
			],
		},
		{
			code: `function generic(): any[] {}`,
			snapshot: `function generic(): any[] {}
                    ~~~
                    Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(): unknown[] {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(): never[] {}`,
				},
			],
		},
		{
			code: `function generic(param: Array<any>): number {}`,
			snapshot: `function generic(param: Array<any>): number {}
                              ~~~
                              Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(param: Array<unknown>): number {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(param: Array<never>): number {}`,
				},
			],
		},
		{
			code: `function generic(param: any[]): number {}`,
			snapshot: `function generic(param: any[]): number {}
                        ~~~
                        Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(param: unknown[]): number {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(param: never[]): number {}`,
				},
			],
		},
		{
			code: `function generic(param: Array<any>): Array<any> {}`,
			snapshot: `function generic(param: Array<any>): Array<any> {}
                              ~~~
                              Unexpected any. Specify a different type.
                                           ~~~
                                           Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(param: Array<unknown>): Array<any> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(param: Array<never>): Array<any> {}`,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(param: Array<any>): Array<unknown> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(param: Array<any>): Array<never> {}`,
				},
			],
		},
		{
			code: `function generic(): Array<Array<any>> {}`,
			snapshot: `function generic(): Array<Array<any>> {}
                                ~~~
                                Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(): Array<Array<unknown>> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(): Array<Array<never>> {}`,
				},
			],
		},
		{
			code: `function generic(): Array<any[]> {}`,
			snapshot: `function generic(): Array<any[]> {}
                          ~~~
                          Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function generic(): Array<unknown[]> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function generic(): Array<never[]> {}`,
				},
			],
		},
		{
			code: `
class Greeter {
  constructor(param: Array<any>) {}
}
      `,
			snapshot: `
class Greeter {
  constructor(param: Array<any>) {}
                           ~~~
                           Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  constructor(param: Array<unknown>) {}
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  constructor(param: Array<never>) {}
}
      `,
				},
			],
		},
		{
			code: `
class Greeter {
  message: any;
}
      `,
			snapshot: `
class Greeter {
  message: any;
           ~~~
           Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  message: unknown;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  message: never;
}
      `,
				},
			],
		},
		{
			code: `
class Greeter {
  message: Array<any>;
}
      `,
			snapshot: `
class Greeter {
  message: Array<any>;
                 ~~~
                 Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  message: Array<unknown>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  message: Array<never>;
}
      `,
				},
			],
		},
		{
			code: `
class Greeter {
  message: any[];
}
      `,
			snapshot: `
class Greeter {
  message: any[];
           ~~~
           Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  message: unknown[];
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  message: never[];
}
      `,
				},
			],
		},
		{
			code: `
class Greeter {
  message: Array<Array<any>>;
}
      `,
			snapshot: `
class Greeter {
  message: Array<Array<any>>;
                       ~~~
                       Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  message: Array<Array<unknown>>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  message: Array<Array<never>>;
}
      `,
				},
			],
		},
		{
			code: `
class Greeter {
  message: Array<any[]>;
}
      `,
			snapshot: `
class Greeter {
  message: Array<any[]>;
                 ~~~
                 Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
class Greeter {
  message: Array<unknown[]>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
class Greeter {
  message: Array<never[]>;
}
      `,
				},
			],
		},
		{
			code: `
interface Greeter {
  message: any;
}
      `,
			snapshot: `
interface Greeter {
  message: any;
           ~~~
           Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Greeter {
  message: unknown;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Greeter {
  message: never;
}
      `,
				},
			],
		},
		{
			code: `
interface Greeter {
  message: Array<any>;
}
      `,
			snapshot: `
interface Greeter {
  message: Array<any>;
                 ~~~
                 Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Greeter {
  message: Array<unknown>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Greeter {
  message: Array<never>;
}
      `,
				},
			],
		},
		{
			code: `
interface Greeter {
  message: any[];
}
      `,
			snapshot: `
interface Greeter {
  message: any[];
           ~~~
           Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Greeter {
  message: unknown[];
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Greeter {
  message: never[];
}
      `,
				},
			],
		},
		{
			code: `
interface Greeter {
  message: Array<Array<any>>;
}
      `,
			snapshot: `
interface Greeter {
  message: Array<Array<any>>;
                       ~~~
                       Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Greeter {
  message: Array<Array<unknown>>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Greeter {
  message: Array<Array<never>>;
}
      `,
				},
			],
		},
		{
			code: `
interface Greeter {
  message: Array<any[]>;
}
      `,
			snapshot: `
interface Greeter {
  message: Array<any[]>;
                 ~~~
                 Unexpected any. Specify a different type.
}
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Greeter {
  message: Array<unknown[]>;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Greeter {
  message: Array<never[]>;
}
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: any;
};
      `,
			snapshot: `
type obj = {
  message: any;
           ~~~
           Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: unknown;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: never;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: Array<any>;
};
      `,
			snapshot: `
type obj = {
  message: Array<any>;
                 ~~~
                 Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: Array<unknown>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: Array<never>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: any[];
};
      `,
			snapshot: `
type obj = {
  message: any[];
           ~~~
           Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: unknown[];
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: never[];
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: Array<Array<any>>;
};
      `,
			snapshot: `
type obj = {
  message: Array<Array<any>>;
                       ~~~
                       Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: Array<Array<unknown>>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: Array<Array<never>>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: Array<any[]>;
};
      `,
			snapshot: `
type obj = {
  message: Array<any[]>;
                 ~~~
                 Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: Array<unknown[]>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: Array<never[]>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string | any;
};
      `,
			snapshot: `
type obj = {
  message: string | any;
                    ~~~
                    Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string | unknown;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string | never;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string | Array<any>;
};
      `,
			snapshot: `
type obj = {
  message: string | Array<any>;
                          ~~~
                          Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string | Array<unknown>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string | Array<never>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string | any[];
};
      `,
			snapshot: `
type obj = {
  message: string | any[];
                    ~~~
                    Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string | unknown[];
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string | never[];
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string | Array<Array<any>>;
};
      `,
			snapshot: `
type obj = {
  message: string | Array<Array<any>>;
                                ~~~
                                Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string | Array<Array<unknown>>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string | Array<Array<never>>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string | Array<any[]>;
};
      `,
			snapshot: `
type obj = {
  message: string | Array<any[]>;
                          ~~~
                          Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string | Array<unknown[]>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string | Array<never[]>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string & any;
};
      `,
			snapshot: `
type obj = {
  message: string & any;
                    ~~~
                    Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string & unknown;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string & never;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string & Array<any>;
};
      `,
			snapshot: `
type obj = {
  message: string & Array<any>;
                          ~~~
                          Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string & Array<unknown>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string & Array<never>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string & any[];
};
      `,
			snapshot: `
type obj = {
  message: string & any[];
                    ~~~
                    Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string & unknown[];
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string & never[];
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string & Array<Array<any>>;
};
      `,
			snapshot: `
type obj = {
  message: string & Array<Array<any>>;
                                ~~~
                                Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string & Array<Array<unknown>>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string & Array<Array<never>>;
};
      `,
				},
			],
		},
		{
			code: `
type obj = {
  message: string & Array<any[]>;
};
      `,
			snapshot: `
type obj = {
  message: string & Array<any[]>;
                          ~~~
                          Unexpected any. Specify a different type.
};
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
type obj = {
  message: string & Array<unknown[]>;
};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
type obj = {
  message: string & Array<never[]>;
};
      `,
				},
			],
		},
		{
			code: `class Foo<t = any> extends Bar<any> {}`,
			snapshot: `class Foo<t = any> extends Bar<any> {}
              ~~~
              Unexpected any. Specify a different type.
                               ~~~
                               Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `class Foo<t = unknown> extends Bar<any> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `class Foo<t = never> extends Bar<any> {}`,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `class Foo<t = any> extends Bar<unknown> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `class Foo<t = any> extends Bar<never> {}`,
				},
			],
		},
		{
			code: `abstract class Foo<t = any> extends Bar<any> {}`,
			snapshot: `abstract class Foo<t = any> extends Bar<any> {}
                       ~~~
                       Unexpected any. Specify a different type.
                                        ~~~
                                        Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `abstract class Foo<t = unknown> extends Bar<any> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `abstract class Foo<t = never> extends Bar<any> {}`,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `abstract class Foo<t = any> extends Bar<unknown> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `abstract class Foo<t = any> extends Bar<never> {}`,
				},
			],
		},
		{
			code: `abstract class Foo<t = any> implements Bar<any>, Baz<any> {}`,
			snapshot: `abstract class Foo<t = any> implements Bar<any>, Baz<any> {}
                       ~~~
                       Unexpected any. Specify a different type.
                                           ~~~
                                           Unexpected any. Specify a different type.
                                                     ~~~
                                                     Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `abstract class Foo<t = unknown> implements Bar<any>, Baz<any> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `abstract class Foo<t = never> implements Bar<any>, Baz<any> {}`,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `abstract class Foo<t = any> implements Bar<unknown>, Baz<any> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `abstract class Foo<t = any> implements Bar<never>, Baz<any> {}`,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `abstract class Foo<t = any> implements Bar<any>, Baz<unknown> {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `abstract class Foo<t = any> implements Bar<any>, Baz<never> {}`,
				},
			],
		},
		{
			code: `new Foo<any>();`,
			snapshot: `new Foo<any>();
        ~~~
        Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `new Foo<unknown>();`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `new Foo<never>();`,
				},
			],
		},
		{
			code: `Foo<any>();`,
			snapshot: `Foo<any>();
    ~~~
    Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `Foo<unknown>();`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `Foo<never>();`,
				},
			],
		},
		{
			code: `
function test<T extends Partial<any>>() {}
const test = <T extends Partial<any>>() => {};
      `,
			snapshot: `
function test<T extends Partial<any>>() {}
                                ~~~
                                Unexpected any. Specify a different type.
const test = <T extends Partial<any>>() => {};
                                ~~~
                                Unexpected any. Specify a different type.
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
function test<T extends Partial<unknown>>() {}
const test = <T extends Partial<any>>() => {};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
function test<T extends Partial<never>>() {}
const test = <T extends Partial<any>>() => {};
      `,
				},
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
function test<T extends Partial<any>>() {}
const test = <T extends Partial<unknown>>() => {};
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
function test<T extends Partial<any>>() {}
const test = <T extends Partial<never>>() => {};
      `,
				},
			],
		},
		{
			code: `
        function foo(a: number, ...rest: any[]): void {
          return;
        }
      `,
			snapshot: `
        function foo(a: number, ...rest: any[]): void {
                                         ~~~
                                         Unexpected any. Specify a different type.
          return;
        }
      `,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
        function foo(a: number, ...rest: unknown[]): void {
          return;
        }
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
        function foo(a: number, ...rest: never[]): void {
          return;
        }
      `,
				},
			],
		},
		{
			code: `type Any = any;`,
			snapshot: `type Any = any;
           ~~~
           Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `type Any = unknown;`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `type Any = never;`,
				},
			],
		},
		{
			code: `function foo5(...args: any) {}`,
			snapshot: `function foo5(...args: any) {}
                       ~~~
                       Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function foo5(...args: unknown) {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function foo5(...args: never) {}`,
				},
			],
		},
		{
			code: `const bar5 = function (...args: any) {};`,
			snapshot: `const bar5 = function (...args: any) {};
                                ~~~
                                Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `const bar5 = function (...args: unknown) {};`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `const bar5 = function (...args: never) {};`,
				},
			],
		},
		{
			code: `const baz5 = (...args: any) => {};`,
			snapshot: `const baz5 = (...args: any) => {};
                       ~~~
                       Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `const baz5 = (...args: unknown) => {};`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `const baz5 = (...args: never) => {};`,
				},
			],
		},
		{
			code: `
interface Qux5 {
  (...args: any): void;
}
      `,
			snapshot: `
interface Qux5 {
  (...args: any): void;
            ~~~
            Unexpected any. Specify a different type.
}
      `,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Qux5 {
  (...args: unknown): void;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Qux5 {
  (...args: never): void;
}
      `,
				},
			],
		},
		{
			code: `function quux5(fn: (...args: any) => void): void {}`,
			snapshot: `function quux5(fn: (...args: any) => void): void {}
                             ~~~
                             Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function quux5(fn: (...args: unknown) => void): void {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function quux5(fn: (...args: never) => void): void {}`,
				},
			],
		},
		{
			code: `function quuz5(): (...args: any) => void {}`,
			snapshot: `function quuz5(): (...args: any) => void {}
                            ~~~
                            Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `function quuz5(): (...args: unknown) => void {}`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `function quuz5(): (...args: never) => void {}`,
				},
			],
		},
		{
			code: `type Fred5 = (...args: any) => void;`,
			snapshot: `type Fred5 = (...args: any) => void;
                       ~~~
                       Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `type Fred5 = (...args: unknown) => void;`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `type Fred5 = (...args: never) => void;`,
				},
			],
		},
		{
			code: `type Corge5 = new (...args: any) => void;`,
			snapshot: `type Corge5 = new (...args: any) => void;
                            ~~~
                            Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `type Corge5 = new (...args: unknown) => void;`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `type Corge5 = new (...args: never) => void;`,
				},
			],
		},
		{
			code: `
interface Grault5 {
  new (...args: any): void;
}
      `,
			snapshot: `
interface Grault5 {
  new (...args: any): void;
                ~~~
                Unexpected any. Specify a different type.
}
      `,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Grault5 {
  new (...args: unknown): void;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Grault5 {
  new (...args: never): void;
}
      `,
				},
			],
		},
		{
			code: `
interface Garply5 {
  f(...args: any): void;
}
      `,
			snapshot: `
interface Garply5 {
  f(...args: any): void;
             ~~~
             Unexpected any. Specify a different type.
}
      `,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
interface Garply5 {
  f(...args: unknown): void;
}
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
interface Garply5 {
  f(...args: never): void;
}
      `,
				},
			],
		},
		{
			code: `declare function waldo5(...args: any): void;`,
			snapshot: `declare function waldo5(...args: any): void;
                                 ~~~
                                 Unexpected any. Specify a different type.`,
			options: {
				ignoreRestArgs: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `declare function waldo5(...args: unknown): void;`,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `declare function waldo5(...args: never): void;`,
				},
			],
		},
		{
			code: `type Keys = keyof any;`,
			snapshot: `type Keys = keyof any;
                  ~~~
                  Unexpected any. Specify a different type.`,
			suggestions: [
				{
					message:
						'Use `PropertyKey` instead, this is more explicit than `keyof any`.',
					output: `type Keys = PropertyKey;`,
				},
			],
		},
		{
			code: `
const integer = <
  TKey extends keyof any,
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
			snapshot: `
const integer = <
  TKey extends keyof any,
                     ~~~
                     Unexpected any. Specify a different type.
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
			suggestions: [
				{
					message:
						'Use `PropertyKey` instead, this is more explicit than `keyof any`.',
					output: `
const integer = <
  TKey extends PropertyKey,
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
				},
			],
		},
		{
			code: `// fixToUnknown: true
type Keys = keyof any;`,
			snapshot: `// fixToUnknown: true
type Keys = keyof any;
                  ~~~
                  Unexpected any. Specify a different type.`,
			output: `// fixToUnknown: true
type Keys = PropertyKey;`,
			options: {
				fixToUnknown: true,
			},
			suggestions: [
				{
					message:
						'Use `PropertyKey` instead, this is more explicit than `keyof any`.',
					output: `// fixToUnknown: true
type Keys = PropertyKey;`,
				},
			],
		},
		{
			code: `
// fixToUnknown: true
const integer = <
  TKey extends keyof any,
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
			snapshot: `
// fixToUnknown: true
const integer = <
  TKey extends keyof any,
                     ~~~
                     Unexpected any. Specify a different type.
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
			output: `
// fixToUnknown: true
const integer = <
  TKey extends PropertyKey,
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
			options: {
				fixToUnknown: true,
			},
			suggestions: [
				{
					message:
						'Use `PropertyKey` instead, this is more explicit than `keyof any`.',
					output: `
// fixToUnknown: true
const integer = <
  TKey extends PropertyKey,
  TTarget extends { [K in TKey]: number },
>(
  target: TTarget,
  key: TKey,
) => {
  /* ... */
};
      `,
				},
			],
		},
		{
			code: `
// fixToUnknown: true
const number: any = 1;
      `,
			snapshot: `
// fixToUnknown: true
const number: any = 1;
              ~~~
              Unexpected any. Specify a different type.
      `,
			output: `
// fixToUnknown: true
const number: unknown = 1;
      `,
			options: {
				fixToUnknown: true,
			},
			suggestions: [
				{
					message:
						'Use `unknown` instead, this will force you to explicitly, and safely assert the type is correct.',
					output: `
// fixToUnknown: true
const number: unknown = 1;
      `,
				},
				{
					message:
						"Use `never` instead, this is useful when instantiating generic type parameters that you don't need to know the type of.",
					output: `
// fixToUnknown: true
const number: never = 1;
      `,
				},
			],
		},
	],
})
