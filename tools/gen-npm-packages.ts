...
await genPackageWithBinary('@golar/', ({ goos, goarch }) => path.join(buildDir, `golar-${goos}-${goarch}`, 'golar'))
await genPackageWithBinary('@golar/astro', ({ goos, goarch }) => path.join(buildDir, `golar-${goos}-${goarch}`, 'golar-astro'))

async function genPackageWithBinary(npmPackageName: string, binaryPathFn: (pl: typeof binariesMatrix[number]) => string) {
	await Promise.all([
	  ...binariesMatrix.map(
	    async ({ goarch, goos, arch, platform }) => {
				const npmPackageNamePlatform = `${npmPackageName}${npmPackageName.endsWith('/') ? '' : '-'}${platform}-${arch}`
				const packageDir = path.join(npmDir, npmPackageNamePlatform.replaceAll('/', '-'))
